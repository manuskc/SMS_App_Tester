...
public class TestForRegexMatch extends TestCaseExecutor{
	
	private String responseMessageRegex;
	private String fromAddressContains;
	private String to;
	private String sendMessage;

	public TestForRegexMatch(Map<String, String> testCase) throws MissingFieldException {
		super(testCase);
		//String to, String sendMessage, String findString, String fromAddressRegex
		this.responseMessageRegex = testCase.get(Constants.TEST_TAGS.RESPONSE_REGEX);
		...
		if(this.responseMessageRegex == null 
				|| this.fromAddressContains == null
				|| this.to == null
				|| this.sendMessage == null) {
			String message = "Missing fields for TestForMessageContains ( id : " + getIdString() +")\n"; //$NON-NLS-1$ //$NON-NLS-2$
			if(this.responseMessageRegex == null) message += Constants.TEST_TAGS.RESPONSE_REGEX + "\n"; //$NON-NLS-1$
	...
	boolean executeTest(String from, String message) {
		if(message.matches(this.responseMessageRegex)) {
			return true;
		}
		return false;
	}
